...
function BeelerReuterRate(φₘ, C₁, C₂, C₃, C₄, C₅, C₆, C₇)
    return (C₁ * exp(C₂ * (φₘ + C₃)) + C₄ * (φₘ - C₅)) / (exp(C₆ * (φₘ + C₃)) + C₇)
end
...
"""
@component function BeelerReuterCalciumDynamics(; name, Caᵢ0 = 1.0e-7)
    @variables begin
        Caᵢ(t) = Caᵢ0
        iCa(t)
    end
    ...
    eqs = [
        D(Caᵢ) ~ -1.0e-7 * iCa + 0.07 * (1.0e-7 - Caᵢ),
    ]
